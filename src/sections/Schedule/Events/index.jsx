...
export default function Events({ items, metadata }) {
  ...
  const renderItems = items.map((item, _, items) => {
    ...
    if (items.some(_item => _item !== item && item.meta.start < _item.meta.end && _item.meta.start < item.meta.end)) {
          ...
          >
            <Card className={eventCard}>
              ...
              <div className={eventContent}>
                ...
                <Text type='meta1' color='white' lineHeight='relaxed' className={eventDetailsMeta}>
                  <span>
                    Location: <Text
                      type='meta1'
                      color='lime'
                    ...
                    >
                      {ORIGINS[origin] ?? 'Somewhere uwu'}
                    </Text>
                  </span>
                  <span>|</span>
                  <span>
                    Host: {item.hosts.map(host => `${host.name} @ ${host.position}`).join(', ') || 'N/A'}
                  </span>
                </Text>
              </div>
            </Card>
