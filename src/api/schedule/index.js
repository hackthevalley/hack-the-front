...
}).base(process.env.EVENTS_AIRTABLE_BASE);
...
export default async function handler(req, res) {
  ...
  const metadata = _metadata.reduce((acc, meta) => {
    switch(meta.fields['Data Type']) {
      ...
      case 'String':
        ...
        break;
      default:
        console.error(`Unknown Type ${meta.fields['Data Type']} for ${meta.fields.Key}`);
        break;
    }
    return acc;
  }, {});
  ...
  const getDateTimeSpan = date => {
    ...
    return ((parseInt(hours) * 60) + parseInt(minutes)) / metadata.MINUTES_PER_CELL;
  };
  const events = _events.reduce((acc, event) => {
    ...
    acc[event.fields['_Day']].push({
      ...
      meta: {
        ...
        end: getDateTimeSpan(event.fields['End']) + (isSameDay ? 0 : metadata.DAY_SPAN),
